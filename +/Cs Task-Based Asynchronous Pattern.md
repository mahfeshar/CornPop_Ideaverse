...
# More
حسنًا، دعنا نبدأ بشرح المحاضرة بالتفصيل، مع تطبيق التعديلات التي تمت إضافتها في الأمثلة كما ذُكرت في الفيديو.

### شرح Task-Based Asynchronous Pattern مع الأمثلة

في بداية الفيديو، ذكر المحاضر أن درس اليوم يتعلق بنمط البرمجة غير المتزامنة Task-Based Asynchronous Pattern (TAP) وأهمية هذا النمط في البرمجة المتقدمة في C#. لكن قبل الخوض في التفاصيل، قام بتصحيح معلومة حول **Thread Cancellation** التي ذُكرت في المحاضرة السابقة، والتي تخص عملية إلغاء الثريدات.

### تصحيح مفهوم Thread Cancellation

ذكر المحاضر أنه عندما نستخدم **CancellationToken** لإلغاء **Thread** عبر **Token.Cancel()**، نحن فقط نرسل طلبًا للإلغاء، لكن المسؤولية الفعلية لتنفيذ الإلغاء تقع على الكود نفسه، فهو الذي يحدد متى وأين يمكن إلغاء العملية. لهذا، يتم فحص **CancellationToken** داخل حلقات أو أماكن محددة من الكود.

### مثال عملي عن إلغاء المهام

في هذا المثال، يتم فحص حالة الإلغاء عند بداية كل دورة من حلقة `for` للتأكد من عدم استمرار المهمة إذا تم طلب الإلغاء:
...
في المثال السابق، إذا طُلب الإلغاء عن طريق **CancellationToken**، سيتم إيقاف العملية بعد التحقق من الطلب داخل الحلقة.

### شرح TAP باستخدام Task.Run()

انتقل المحاضر بعد ذلك لشرح كيفية استخدام **Task.Run()** لتنفيذ العمليات بشكل غير متزامن. في المثال التالي، نرى كيف يتم تشغيل مهام متعددة بدون انتظار بعضها البعض (أي بدون حجب **Blocking** للثريد الأساسي):
...
أثناء الشرح، أكد المحاضر أن **Task.Run()** يُستخدم عادة لبدء مهمة في الخلفية، وأنه يمكن استخدامه في سيناريوهات متعددة، لكنه ليس الأسلوب الأكثر استخدامًا دائمًا، حيث توجد طرق أخرى سنشرحها لاحقًا.

### شرح Synchronous vs Asynchronous

قدم المحاضر شرحًا عن الفرق بين **Synchronous** و **Asynchronous**. في البرمجة المتزامنة **Synchronous**، يتم تنفيذ الأسطر واحدًا تلو الآخر، وكل سطر ينتظر السطر الذي يسبقه لينتهي، بينما في البرمجة غير المتزامنة **Asynchronous**، يمكن تنفيذ عدة أسطر معًا بدون انتظار.
...
لجعل الكود غير متزامن بشكل فعال، قدم المحاضر استخدام **async** و **await** في كتابة الميثودات، وأوضح كيف يتم تحويل ميثود عادية إلى ميثود غير متزامنة. مثلًا، في هذا المثال:
...
قام المحاضر بتوضيح تنفيذ المهام غير المتزامنة عبر إضافة **Breakpoint** داخل الكود. مثلاً، عند تشغيل **Task.Run()** ووجود عدة مهام، يمكن رؤية أن المهام يتم تنفيذها في **Thread** منفصل لكل واحدة منها. وأوضح الفرق بين التنفيذ المتزامن وغير المتزامن عبر مقارنة النتائج:
...
لتشغيل عدة مهام في نفس الوقت والانتظار حتى تنتهي جميعها، قام المحاضر بشرح **Task.WhenAll()** التي تسمح بانتظار إتمام جميع المهام:
...
أثناء الشرح، أضاف المحاضر **Task.Delay()** داخل الميثودات لتوضيح كيف يتم تأخير المهام بدون حجب:
