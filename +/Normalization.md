...
- الخانة "Members" فيها أكتر من اسم في نفس الخلية، يعني دي خالفت شرط القيم الفردية.  
- مفيش مفتاح أساسي واضح يميز كل صف عن الآخر.  

#### **لماذا هذا الجدول غير محقق لـ 1NF؟**
1. الخلية تحتوي على قيم متعددة (Tom, John في نفس الخلية).
2. لو حبينا نعمل Query نجيب اسم شخص معين، هنحتاج نعمل معالجة إضافية عشان نفصل القيم.
...
### **أمثلة إضافية على مشاكل التكرار**
#### **المثال الأول: تغيير بيانات عضو في فريق**
لو حبينا ننقل "John" من فريق "Front End" إلى "Back End"، كان الجدول القديم هيحتاج معالجة نصوص لفصل القيم وإعادة تخزينها. بعد التعديل للـ 1NF، التغيير أصبح بسيط جدًا:
- بس نعدل قيمة Team للصف الخاص بـ "John" ونخليها "Back End".

#### **المثال الثاني: البحث عن بيانات محددة**
...
1. **استخدام ترتيب الصفوف لتمثيل معلومات:**  
   مثال: لو قولت إن الصف الأول بيعبر عن العضو الأقدم في الفريق، والصف الثاني عن العضو الجديد، ده خطأ. لازم تضيف عمود صريح يعبر عن الأقدمية، زي:
...
2. **عدم وجود Primary Key:**  
   لو الجدول مش فيه مفتاح أساسي زي "ID"، ممكن تحصل مشاكل تكرار. الحل هو إضافة عمود جديد يميز كل صف زي "MemberID":
...
1. كل خلية تحتوي على قيمة واحدة فقط.
2. مفيش Multi-Valued Cells.
3. الترتيب مش عامل مهم لتمثيل البيانات.
4. وجود مفتاح أساسي يميز كل صف.

---

### **الفائدة من 1NF**
- تنظيم البيانات بطريقة تخليك تقدر تعمل عليها Queries بسهولة.
- تمنع المشاكل اللي بتنتج من التكرار.
- تسهل عملية التعديل، الإضافة، أو الحذف للبيانات بدون أخطاء.
**الشروط:**
...
الشكل ده مش محقق لـ 1NF لأنه عندنا **Multi-Valued Cells**. 
الحل هو فصل كل عضو في صف منفصل:  
...
| Back End   | Jenny   |  
| Back End   | Bob     |  
##### ليه نبعد عن الـ Multi-Valued
- عشان تعملها Query صعب، فأنت بتاخد الـ Cell كلها وبتعملها Parsing 
- ترتيب الشخص اللي بيدور برضو
- لو واحد محتاج يغير التيم بتاعه لازم أدور على اسمه الأول وبعدين أمسحه وأحطه في الصف التاني
---
